...
public class CameraScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        box_y0 = playerPos.y - boxSizeX ;
        box_y1 = playerPos.y + boxSizeX ;

        Vector3 bottomLeft = thiscamera.ViewportToWorldPoint(new Vector3(0,0,0));

        Vector3 topRight = thiscamera.ViewportToWorldPoint(new UnityEngine.Vector3(1,1,0));

        screenX0 = bottomLeft.x;
        screenX1 = topRight.x;

        if (box_x0 <screenX0)
            camX = playerPos.x + 0.5f * (screenX1 - screenX0) - boxSizeY;
        ...
        transform.position = new UnityEngine.Vector3(camX, camY, cameraPos.z);
    }
}
